...
import com.example.easyRide.easyRideApplication;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, classes = easyRideApplication.class)
@ActiveProfiles({"test"})
//@DirtiesContext(classMode = DirtiesContext.ClassMode.BEFORE_CLASS) // RESET APPLICATION CONTEXT AND RESET DB
@Sql(scripts = {
        "classpath:/dbH2/02-truncate.sql",
        "classpath:/dbH2/03-insert-table.sql",
...
public abstract class BaseIntegrationTest {
    ...
    @BeforeEach
    void setup() {
        UserLoginDTO given = UserLoginDTO.builder()
                .firstName("Moustapha")
                .lastName("Ndiaye")
                .build();
        UserLoginInfoDTO response = given()
                .port(port)
